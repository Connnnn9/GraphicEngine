...
void UIButtonComponent::Initialize()
{
	...
	{
		if (!mButtonStatesTexture[i].empty())
		{
			mButtonStates[i].Initialize(mButtonStatesTexture[i]);
		}
	}
...
void UIButtonComponent::Render()
{
	...
	if (!mButtonStatesTexture[buttonState].empty())
	{
...
void UIButtonComponent::Deserialize(const rapidjson::Value& value)
{
	for (uint32_t i = 0; i < static_cast<uint32_t>(ButtonState::Count); ++i)
	{
		...
		switch (state)
		{
		case ButtonState::Default: buttonStateStr = "Default";
			break;
		case ButtonState::Hover: buttonStateStr = "Hover";
			break;
		case ButtonState::Click:buttonStateStr = "Click";
			break;
		case ButtonState::Disabled:buttonStateStr = "Disabled";
			break;
		default:
			break;
		}
		...
		{
			mButtonStatesTexture[i] = buttonStateObj["Texture"].GetString();
		}
		...
		if (buttonStateObj.HasMember("Position"))
		{
		...
		if (buttonStateObj.HasMember("Rotation"))
		{
		...
		if (buttonStateObj.HasMember("Pivot"))
		{
